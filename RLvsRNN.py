...
class FFNAgent(Player):
    ...
    def update_policy(self):
        ...
        loss_terms = [-log_prob * R for log_prob, R in zip(self.log_probs, returns)]
        if not loss_terms:
            return
...
# Curriculum Training
def curriculum_train(ffn, rnn, curriculum, phase_episodes=2000, rounds=150):
    for phase_num, phase_pool in enumerate(curriculum, 1):
        ...
        for ep in range(phase_episodes):
            ...
            run_match({'FFN': ffn, 'RNN': rnn}, rounds, training=True, update=False)  # Optional cross-play
            ffn.update_policy()
            rnn.update_policy()
            if (ep + 1) % 500 == 0:
...
if __name__ == '__main__':
    ...
    curriculum = [
        ...
        [AlwaysDefect(), RandomStrategy(), ffn, rnn]
    ]

    print("Training agents using curriculum...")
    curriculum_train(ffn, rnn, curriculum)

    strategies = [TitForTat(), GrimTrigger(), Pavlov(), AlwaysCooperate(), AlwaysDefect(), RandomStrategy()]
