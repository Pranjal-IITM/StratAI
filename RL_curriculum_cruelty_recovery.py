...
# Match + Curriculum setup (unchanged except curriculum)
...
def curriculum_train(ffn, rnn, curriculum, phase_episodes=1000, rounds=200):
...
if __name__ == '__main__':
    ...
    curriculum = [
        [AlwaysCooperate(), TitForTat()],  # Learn to cooperate
        [Pavlov(), TitForTat()],           # Learn forgiveness
        [AlwaysDefect(), RandomStrategy()],# Get cooked by cruelty
        [GrimTrigger(), SuspiciousTitForTat()],  # Learn forgiveness again
        [TitForTat(), GrimTrigger(), RandomStrategy(), AlwaysDefect(), Pavlov(), SuspiciousTitForTat()]  # All-out brawl
        ]
    print("Training agents using improved curriculum with REINFORCE baseline...")
    curriculum_train(ffn, rnn, curriculum)
    ffn.policy_net.load_state_dict(torch.load("ffn_model.pth"))
    rnn.rnn.load_state_dict(torch.load("rnn_rnn.pth"))
    rnn.fc.load_state_dict(torch.load("rnn_fc.pth"))
...
def evaluate_and_plot_individual(agent_ffn, agent_rnn, strategies, rounds=200):
    def plot_match(agent_name, strategy_name, moves_agent, moves_strategy, avg_a, avg_s):
        ...
        plt.legend()
        plt.tight_layout()
        filename = f"{agent_name}_vs_{strategy_name}.png"
    ...
    for strat in strategies:
        ...
        plot_match("FFN", strat_name, moves_ffn, moves_s1,
                   rewards_f['FFN'] / rounds, rewards_f['S'] / rounds)
        ...
        plot_match("RNN", strat_name, moves_rnn, moves_s2,
                   rewards_r['RNN'] / rounds, rewards_r['S'] / rounds)
    ...
    plot_match("FFN", "RNN", moves_ffn, moves_rnn,
               rewards_both['FFN'] / rounds, rewards_both['RNN'] / rounds)
    plot_match("RNN", "FFN", moves_rnn, moves_ffn,
               rewards_both['RNN'] / rounds, rewards_both['FFN'] / rounds)
...
# Evaluate after training
    strategies = [TitForTat(), GrimTrigger(), Pavlov(), AlwaysCooperate(), AlwaysDefect(), SuspiciousTitForTat()]
    print("Evaluating agents against fixed strategies...")
    evaluate_and_plot_individual(ffn, rnn, strategies)
    print("Done. Plots saved.")
